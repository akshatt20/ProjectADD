...
class PatientDeactPrescriptionActivity : AppCompatActivity() {
    ...
    private fun setUpRCV(prescriptions: ArrayList<Prescription>) {
        var deactivatedPrescriptions : ArrayList<Prescription> = ArrayList()
        for(i in 0 .. prescriptions.size - 2) {
            deactivatedPrescriptions[i] = prescriptions[i]
        }
        if(deactivatedPrescriptions.size == 0) {
            Toast.makeText(this, "No deactivated Prescriptions", Toast.LENGTH_SHORT).show()
            return
        }
        binding.rcvPrescriptions.adapter = RCVPrescriptionsAdapter(this,prescriptions,object : RCVPrescriptionsAdapter.PrescriptionListener{
            override fun onPrescriptionClicked(prescription: Prescription) {
                var intent = Intent(this@PatientDeactPrescriptionActivity,ActivityPrescriptionDetails::class.java)
                intent.putExtra("PRESCRIPTION",Gson().toJson(prescription));
